...
const user = (m.isGroup ? 
  participants.find((u) => conn.decodeJid(u.jid ?? u.lid ?? u.id) === m.sender
) : {}) || {};
const bot = (m.isGroup ? 
  participants.find((u) => conn.decodeJid(u.jid ?? u.lid ?? u.id) === (this.user.jid ?? this.user.lid)
) : {}) || {};
...
if (data.exists)
m.reply(`*⚠️ 『خـطـأ فـي تـنـفـيذ الأمـر』 ⚠️*\n\n*📑 『الإضـافـة』 :* ${name}\n*👤 『المستخـدم』 :* ${m.sender}\n*🚀 『الأمـر』 :* ${m.text}\n\n\`\`\`${format(e)}\`\`\`\n`.trim(), data.jid)                               
}}
}
...
let chat = global.db.data.chats[m.chat]
let user = global.db.data.users[m.sender]
if (name != 'رفع-الحظر.js' && chat?.isBanned)
return
...
this.reply(m.chat, `💎 ألماساتك اليومية قد اِنتهت ⚠️\n\n🛒 من فضلك اِشترِ الألماس عبر كتابة:\n${usedPrefix}buy أو ${usedPrefix}buyall`, m)
continue
}
if (plugin.level > _user.level) {
this.reply(m.chat, `🐉 هـذا الأمــر يـتـطـلــب الـوصـول إلــى الـمسـتـوى *${plugin.level}*`, m)
continue
}
...
await conn.reply(res, `*⚠️ [ خــطــأ فــي تــنــفــيــذ الأمــر ] ⚠️*\n\n*📑 [ الإضـافــة ] :* ${m.plugin}\n*👤 [ المـستخـدم ] :* ${m.sender}\n*🚀 [ الأمــر ] :* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\`\n\n`)
 
 m.reply(`*⚠️ [ خــطــأ فــي تــنــفــيــذ الأمــر ] ⚠️*\n\n*📑 [ الإضـافــة ] :* ${m.plugin}\n*👤 [ المـستخـدم ] :* ${m.sender}\n*🚀 [ الأمــر ] :* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\`\n\n`.trim(), data.jid)
}
m.reply(text)
}
} finally {
...
if (m.diamond)
m.reply(`💎 تـمَ اِسـتِخدام ألماسـة ${+m.diamond}`)
}
break
}
}
...
if (db.data.chats[m.chat].reaction && m.text.match(/(الله|محمد|سلام|حب|جميل|مصر|عربي|مرحبا|اوبيتو|مصطفي|ايانوكوجي|ياسين|ميكو|ميكو بوت)/gi)) {
let emot = pickRandom([
"😀", "😃", "😄", "😁", "😆", "🥹", "😅", "😂", "🤣", "🥲", "☺️", "😊", "😇", "🙂", "🙃", "😉", "😌",
"😍", "🥰", "😘", "😗", "😙", "😚", "😋", "😛", "😝", "😜", "🤪", "🤨", "🧐", "🤓", "😎", "🥸", "🤩",
"🥳", "😏", "😒", "😞", "😔", "😟", "😕", "🙁", "☹️", "😣", "😖", "😫", "😩", "🥺", "😢", "😭", "😤",
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://files.catbox.moe/160o43.jpg'
                    let ppgp = 'https://files.catbox.moe/160o43.jpg'
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Desconocido') :
                            ...
                            let wel = API('fgmods', '/api/welcome', {
                                ...
                                profile: pp,
                                background: 'https://files.catbox.moe/160o43.jpg'
                            }, 'apikey')
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] تنبيه - اشعار', body: 'فولو لقناة الدعم miku bot', thumbnail: imagen1, sourceUrl: 'https://whatsapp.com/channel/0029VaQMPz0DTkKCSNUWOF3m' } } } }, aa);
